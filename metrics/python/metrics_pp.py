...
class metrics_pp(object):
    """docstring for metrics_pp"""
    def __init__(self, filepath, dataset_name, algorithm_name, decay_factor=0):
        super(metrics_pp, self).__init__()
    ...
    def calc(self, triple, position, filename):
        recommendscore = {}
        user_predictitem = {}
        user_rankingscore = {}
        user_auc = {}
...
if __name__ == '__main__':
    ...
    import time
    t0=time.clock()
    mymetric = metrics_pp(filepath="../../CF/UCF/offline_results/ml-100k/ucf/", dataset_name="ml-100k", algorithm_name="ucf", decay_factor=decay_factor)
    mymetric.import_probeset()
    # mymetric.import_recommendscore("tra_ucf-user_recommendscore_%s.txt"%decay_factor)
    mymetric.import_recommendscore("ucf-user_recommendscore.txt")
    mymetric.test()
    print "metrics_pp costs %ss"%(time.clock()-t0)