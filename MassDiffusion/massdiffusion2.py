...
class MassDiffusion2(object):
    ...
    def create_ui_matrix(self, method="online"):
        ...
        self.user_degree = scipy.sparse.csr_matrix(self.ui_matrix.sum(0))
        self.item_degree = scipy.sparse.csr_matrix(self.ui_matrix.sum(1))

    def diffusion(self, user_id):
        resourse = {}
        user_item_list = self.ui_matrix[:, user_id].toarray().tolist()
        uncollected_item_id = 0
        for each_uncollected in user_item_list:
            if each_uncollected == 0:
                total_resourse = 0
                collected_item_id = 0
                for each_collected in user_item_list:
                    if each_collected != 0:
                        tmp = self.ui_matrix[uncollected_item_id, :].multiply(self.ui_matrix[collected_item_id, :])
                        if tmp.sum(1)[0,0] != 0:
                            total_resourse += self.item_degree[collected_item_id, 0]*(tmp/self.user_degree).sum(1)[0,0]
                collected_item_id += 1
                resourse[uncollected_item_id] = total_resourse
            uncollected_item_id += 1
        return resourse



    def single_predict(self, user_id):
        fVector = self.diffusion(user_id)
        fVector =  sorted(fVector.iteritems(), key=lambda d:d[1], reverse = True)
        try:
            single_rankingscore = (fVector.index((self.testSet[user_id][0], dict(fVector)[self.testSet[user_id][0]])) + 1)/len(fVector)
        except Exception, e:
            print e
            sys.exit()
        return [fVector[0][0], single_rankingscore]
    ...
    def predict(self, scope, groupid):
        ...
        for user in range(scope[0], scope[1]):
            [predict_item, single_rankingscore] = self.single_predict(int(user))
            rangkingscore += single_rankingscore
            predict_itemdata[user] = predict_item
...
if __name__ == '__main__':
    md = MassDiffusion2(filepath="../../../data/shujuchuli_0618/fengniao_chengyu_0618.txt", dataset_name="fengniao", split_traintest="yes")
    t0 = time.clock()
